...
function getHousePositions(center, halfSize) {
  // Carefully positioned based on reference image
  const diamondMid = halfSize;              // Distance to midpoint of outer square
  const cornerDist = halfSize * 0.55;       // Distance for corner triangle planets
  const sideInner = halfSize * 0.6;         // For side section planets
  const numDist = halfSize * 0.22;          // Distance for house numbers from center

  return {
    // House numbers positioned along lines near center
    houseNumbers: {
      // Central diamond area numbers
      1: { x: center + numDist + 5, y: center + 5 },           // Right of center
      4: { x: center, y: center - numDist - 5 },               // Above center
      7: { x: center - numDist - 5, y: center + 5 },           // Left of center
      10: { x: center, y: center + numDist + 10 },             // Below center

      // Upper diamond section
      2: { x: center - 15, y: center - numDist - 25 },         // Left side of top diamond
      12: { x: center + 15, y: center - numDist - 25 },        // Right side of top diamond

      // Side diagonal sections
      3: { x: center - numDist - 30, y: center - numDist - 15 }, // Upper left diagonal
      5: { x: center - numDist - 30, y: center - 5 },            // Left upper diagonal

      // Lower sections
      6: { x: center - numDist - 30, y: center + numDist + 20 }, // Lower left diagonal
      8: { x: center - numDist - 10, y: center + numDist + 30 }, // Left lower diagonal

      9: { x: center + numDist + 30, y: center + numDist + 30 }, // Lower right far
      11: { x: center + numDist + 10, y: center + numDist + 25 } // Lower right
    },

    // Planet positions in clear open areas
    planets: {
      // Four corner triangles (large open areas)
      3: { x: center - cornerDist * 1.1, y: center - cornerDist * 1.1 },  // Upper left triangle
      6: { x: center - cornerDist * 1.1, y: center + cornerDist * 1.1 },  // Lower left triangle (Mo Sa)
      9: { x: center + cornerDist * 1.1, y: center + cornerDist * 1.1 },  // Lower right triangle
      12: { x: center + cornerDist * 1.1, y: center - cornerDist * 1.1 }, // Upper right triangle (Ke)

      // Four side sections (in diamond between center and corners)
      1: { x: center + sideInner, y: center },                            // Right side (As)
      4: { x: center, y: center - sideInner },                            // Top side
      7: { x: center - sideInner, y: center },                            // Left side (Mo Sa area)
      10: { x: center, y: center + sideInner },                           // Bottom side (Ma Ju Ve)

      // Four smaller sections (between diamond edges and corners)
      2: { x: center + cornerDist * 0.5, y: center - cornerDist * 0.9 },  // Upper right
      5: { x: center - cornerDist * 0.5, y: center - cornerDist * 0.9 },  // Upper left
      8: { x: center - cornerDist * 0.5, y: center + cornerDist * 0.9 },  // Lower left (Ra)
      11: { x: center + cornerDist * 0.5, y: center + cornerDist * 0.9 }  // Lower right (Su Me)
    }
  };
}
