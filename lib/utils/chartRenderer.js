...
function getHousePositions(center, halfSize) {
  // Positions based on North Indian chart structure with correct line divisions
  const cornerOffset = halfSize * 0.6;      // For 4 large corner triangles
  const sideOffset = halfSize * 0.75;       // For 4 side sections (between corner and diamond)
  const innerOffset = halfSize * 0.35;      // For 4 inner diamond sections

  return {
    // House numbers near line intersections
    houseNumbers: {
      // Corner triangle sections
      3: { x: center - innerOffset - 10, y: center - cornerOffset + 30 },    // Upper left
      6: { x: center - cornerOffset + 30, y: center + innerOffset + 10 },    // Lower left
      9: { x: center + innerOffset + 10, y: center + cornerOffset - 30 },    // Lower right
      12: { x: center + cornerOffset - 30, y: center - innerOffset - 10 },   // Upper right

      // Side sections (along diamond edges)
      2: { x: center + innerOffset - 15, y: center - innerOffset - 20 },     // Upper right diamond edge
      5: { x: center - innerOffset + 15, y: center - innerOffset + 20 },     // Upper left diamond edge
      8: { x: center - innerOffset + 15, y: center + innerOffset - 20 },     // Lower left diamond edge
      11: { x: center + innerOffset - 15, y: center + innerOffset + 20 },    // Lower right diamond edge

      // Inner central sections
      1: { x: center + 25, y: center + 5 },                                  // Right center
      4: { x: center - 5, y: center - 25 },                                  // Top center
      7: { x: center - 25, y: center - 5 },                                  // Left center
      10: { x: center + 5, y: center + 25 }                                  // Bottom center
    },

    // Planet positions in clear open areas
    planets: {
      // Large corner triangles (outside the diamond)
      3: { x: center - cornerOffset, y: center - cornerOffset },             // Top-left corner
      6: { x: center - cornerOffset, y: center + cornerOffset },             // Bottom-left corner (Mo Sa in ref)
      9: { x: center + cornerOffset, y: center + cornerOffset },             // Bottom-right corner
      12: { x: center + cornerOffset, y: center - cornerOffset },            // Top-right corner (Ke in ref)

      // Side sections (between diamond and corners)
      2: { x: center + sideOffset * 0.4, y: center - sideOffset * 0.75 },    // Upper right side
      5: { x: center - sideOffset * 0.4, y: center - sideOffset * 0.75 },    // Upper left side
      8: { x: center - sideOffset * 0.4, y: center + sideOffset * 0.75 },    // Lower left side (Ra in ref)
      11: { x: center + sideOffset * 0.4, y: center + sideOffset * 0.75 },   // Lower right side (Su Me in ref)

      // Inner diamond sections
      1: { x: center + sideOffset * 0.7, y: center },                        // Right (As in ref)
      4: { x: center, y: center - sideOffset * 0.7 },                        // Top
      7: { x: center - sideOffset * 0.7, y: center },                        // Left
      10: { x: center, y: center + sideOffset * 0.7 }                        // Bottom (Ma Ju Ve in ref)
    }
  };
}
