...
function drawHouseContent(houseNum, position, signNum, planets) {
  ...
  // House number (small, subtle) - larger for bigger chart
  content += `<text x="${x}" y="${y - 30}"
    font-size="16"
    font-family="Arial, sans-serif"
    text-anchor="middle"
    fill="#666">${houseNum}</text>`;

  // Sign abbreviation - larger for bigger chart
  ...
  // Planets (if any) - larger for bigger chart
  if (planets && planets.length > 0) {
    const planetText = planets.join(' ');
    content += `<text x="${x}" y="${y + 28}"
      font-size="22"
      font-family="Arial, sans-serif"
      text-anchor="middle"
      font-weight="bold"
