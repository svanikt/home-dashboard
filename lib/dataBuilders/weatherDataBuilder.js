...
/**
 * Build weather dashboard data from all available services
 ...
 */
async function buildWeatherDashboardData(dashboardConfig, req, logger = console) {
  const now = new Date();
  ...
  const timezone = weatherData.timezone || 'America/New_York';
  ...
  try {
    ...
    logger.info?.('[DataBuilder] Message board:', message.isGreeting ? `greeting: "${message.text}"` : 'custom message');
  } catch (error) {
