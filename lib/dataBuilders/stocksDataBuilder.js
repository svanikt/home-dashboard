...
const { VedicRishiService } = require('../../services/vedicRishiService');
...
async function buildStocksDashboardData(dashboardConfig, req, logger = console, forceRefresh = false) {
  ...
  const vedicService = dashboardConfig.services.vedic?.enabled
    ? new VedicRishiService(1440) // 24 hour cache for vedic data
    : null;
  ...
  if (vedicService) {
    try {
      const vedicConfig = dashboardConfig.services.vedic;
