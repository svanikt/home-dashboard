...
const { VedicAstrologyService } = require('../../services/vedicAstrologyService');
...
async function buildStocksDashboardData(dashboardConfig, req, logger = console, forceRefresh = false) {
  ...
  const vedicService = dashboardConfig.services.vedic?.enabled
    ? new VedicAstrologyService(1440) // 24 hour cache for vedic data
    : null;
  ...
  // Fetch Vedic astrology data (OPTIONAL)
  let vedicData = null;
  let vedicStatus;
  if (vedicService) {
    try {
      const vedicConfig = {
        birthTime: dashboardConfig.services.vedic?.birthTime,
        birthLocation: dashboardConfig.services.vedic?.birthLocation,
      };
      const result = await vedicService.getData(vedicConfig, logger, forceRefresh);
      vedicData = result.data;
      vedicStatus = result.status;
      logger.info?.('[StocksDataBuilder] Vedic astrology data loaded');
    } catch (error) {
      logger.info?.('[StocksDataBuilder] Vedic astrology service unavailable (optional):', error.message);
      vedicStatus = vedicService.getStatus();
    }
  }
  ...
  data._serviceStatuses = {
    stocks: stockStatus,
    vedic: vedicStatus || { enabled: false },
